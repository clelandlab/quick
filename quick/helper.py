...
def fitResonator(F, S, fit="circle", p0=[None, None, None, None]):
    ...
    axes[1].text(0.05, 0.05, r"$f = %.5e$" % p[2], ha="left", va="bottom", transform=axes[1].transAxes)
    axes[1].text(0.75, 0.29, "Error:", ha="left", va="bottom", transform=axes[1].transAxes)
    axes[1].text(0.75, 0.21, r"$\Delta Q_i = %.1e$" % perr[0], ha="left", va="bottom", transform=axes[1].transAxes)
    axes[1].text(0.75, 0.13, r"$\Delta Q_c = %.1e$" % perr[1], ha="left", va="bottom", transform=axes[1].transAxes)
    axes[1].text(0.75, 0.05, r"$\Delta f = %.1e$" % perr[2], ha="left", va="bottom", transform=axes[1].transAxes)
    axes[1].grid()
    ...
    axes[2].text(0.05, 0.87, r"$\chi^2/n = {:.1e}$".format(rchi2), ha="left", va="top", transform=axes[2].transAxes)
